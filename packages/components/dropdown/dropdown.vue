<template>
	<m-popup :show="show" @click.native.self="hide" :overlay-color="overlayColor" :z-index="zIndex" :timeout="timeout"
		...
		<div class="mvi-dropdown">
			<div :disabled="itemDisabled(item)" :class="dropdownItemClass(item,index)" v-for="(item,index) in options"
				:key="'item-'+index" @click="doSelect(item,index)" :style="dropdownItemStyle(item,index)">

<template>
	<div class="mvi-page" v-on="listeners">
		<div v-if="firstText || firstIconType || firstIconUrl" :disabled="page==1" @click="pageFirst"
			:class="['mvi-page-first',(active&&page!=1)?'mvi-page-active':'']"
		...
		<div v-if="prevText || prevIconType || prevIconUrl" :disabled="page==1" @click="pagePrev"
			:class="['mvi-page-prev',(active&&page!=1)?'mvi-page-active':'']"
			:style="{color:(page==1?'':(color?color:''))}">
		...
		<div class="mvi-page-numbers">
			...
			<div class="mvi-page-numbers-items" v-else>
				...
				<div v-if="total<=overNumber"
					:class="['mvi-page-numbers-item',page==item?'mvi-page-number-active':'',(active&&page!=item)?'mvi-page-active':'']"
					v-for="(item,index) in total" v-text="item" :key="'page-'+index" @click="toPage(item)"
					:style="pageStyle(item)"></div>
				<!-- total超过overNumber -->
				<div v-if="total>overNumber && page > (overNumber-1)/2+1"
					:class="['mvi-page-numbers-item',active?'mvi-page-active':'']" @click="toPage(page-(overNumber-1))"
					:style="{color:color?color:''}">...</div>
				<div v-if="total>overNumber"
					:class="['mvi-page-numbers-item',page==item?'mvi-page-number-active':'',(active&&page!=item)?'mvi-page-active':'']"
					v-for="(item,index) in arry" v-text="item" :key="'page2-'+index" @click="toPage(item)"
					:style="pageStyle(item)"></div>
				<div v-if="total>overNumber && page < (total - (overNumber-1)/2)"
					:class="['mvi-page-numbers-item',active?'mvi-page-active':'']" @click="toPage(page+(overNumber-1))"
					:style="{color:color?color:''}">...</div>
			</div>
		</div>
		<div v-if="nextText || nextIconType || nextIconUrl" :disabled="page==total" @click="pageNext"
			:class="['mvi-page-next',(active&&page!=total)?'mvi-page-active':'']"
		...
		<div v-if="lastText || lastIconType || lastIconUrl" :disabled="page==total" @click="pageLast"
			:class="['mvi-page-last',(active&&page!=total)?'mvi-page-active':'']"
			:style="{color:(page==total?'':(color?color:''))}">

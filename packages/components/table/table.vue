<template>
	<div v-on="listeners" :class="['mvi-table', outBorder ? 'mvi-table-border' : '']">
						...
						<th ref="ths" :class="tableHeaderThClass" v-for="(item, index) in columnsData" :key="'table-column-' + index">
							<div>
								...
								<span class="mvi-table-sortable" v-if="item.sortable">
									<m-icon
										@click="sortAsc($event, item)"
										:class="['mvi-table-sortable-icon', item.key == sortBy && sortMethod == 'asc' ? 'mvi-table-sortable-icon-active' : '']"
										:type="(sortIcon && sortIcon.length == 2 && sortIcon[0])? sortIcon[0] : 'caret-up'"
										:color="sortActiveColor ? (item.key == sortBy && sortMethod == 'asc' ? sortActiveColor : null) : null"
									/>
									<m-icon
										@click="sortDesc($event, item)"
										:class="['mvi-table-sortable-icon', item.key == sortBy && sortMethod == 'desc' ? 'mvi-table-sortable-icon-active' : '']"
										:type="(sortIcon && sortIcon.length == 2 && sortIcon[1])? sortIcon[1] : 'caret-down'"
										:color="sortActiveColor ? (item.key == sortBy && sortMethod == 'desc' ? sortActiveColor : null) : null"
									/>
								</span>
							</div>
						</th>
		...
		<div v-else class="mvi-table-body" :style="bodyStyle" ref="body">
			<table cellpadding="0" cellspacing="0">
				<colgroup><col ref="bodyCols" :style="colgroupStyle(item, index)" v-for="(item, index) in columnsData" :key="'table-body-group-' + index" /></colgroup>
				<tbody>
					<tr v-for="(item, index) in sortData" :key="'table-data-' + index" :class="bodyTrClass(item, index)">
						<td
							...
							v-for="(item2, index2) in columnsData"
						...
						>
							<m-checkbox
								...
								@model-change="selectCheck"
								:icon-color="item2.iconColor ? item2.iconColor : null"
								:fill-color="item2.fillColor ? item2.fillColor : null"
								:disabled="item.checkDisabled ? true : false"
							></m-checkbox>

<template>
	<div v-on="listeners" :class="['mvi-table', outBorder ? 'mvi-table-border' : '']">
						...
						<th ref="ths" :class="tableHeaderThClass" v-for="(item, index) in columnsData"
							...
							<div>
								...
								<span class="mvi-table-sortable" v-if="item.sortable">
									...
									<m-icon @click="sortDesc($event, item)"
		...
		<div v-else class="mvi-table-body" :style="bodyStyle" ref="body">
			<table cellpadding="0" cellspacing="0">
				<colgroup>
					<col ref="bodyCols" :style="colgroupStyle(item, index)" v-for="(item, index) in columnsData"
						:key="'table-body-group-' + index" />
				</colgroup>
				<tbody>
					<tr v-for="(item, index) in sortData" :key="'table-data-' + index"
						:class="bodyTrClass(item, index)">
						<td :class="[

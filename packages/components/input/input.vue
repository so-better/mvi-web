<template>
	<div :disabled="disabled"
		:class="['mvi-input-container','mvi-input-container-'+size,border?'mvi-input-border':'',(showWordLimit && maxlength>0)?'mvi-input-container-words':'',required?'mvi-input-required':'']"
		...
		<textarea v-if="type=='textarea'" :placeholder="placeholder" :maxlength="maxlength" :disabled="disabled"
			:readonly="readonly" :autofocus="autofocus" class="mvi-textarea" v-on="listeners" v-model="realValue"
			ref="textarea" :rows="rowsFilter" :name="name" :style="textareaStyle" @focus="getFocus" @blur="getBlur"
			autocomplete="off"></textarea>
		<!-- 输入框 -->
		<input v-else :type="inputType" :inputmode="computedInputMode" :placeholder="placeholder" :disabled="disabled"
			:readonly="readonly || isDatePicker" :autofocus="autofocus" class="mvi-input" v-on="listeners"
			v-model="realValue" @click="callDate" ref="input" :name="name" :style="inputStyle" @focus="getFocus"
			@blur="getBlur" autocomplete="off" :maxlength="(isDatePicker?-1:maxlength)">
		<!-- 清除图标 -->
		...
		<div @click="rightClick" class="mvi-input-right-icon" v-if="$slots.right || (rightIconUrl || rightIconType)">
			...
			<m-icon v-else-if="rightIconUrl || rightIconType" ref="rightIcon" :type="rightIconType" :url="rightIconUrl"
				:spin="rightIconSpin" :size="rightIconSize" :color="rightIconColor" />
		</div>
		<!-- 显示文字长度限制 -->

<template>
	<m-popup popup-color="#fff" :show="show" @click.native.self="hide" :overlay-color="overlayColor" :z-index="zIndex"
		:timeout="timeout" placement="bottom" :round="round" :local="local" :use-padding="usePadding" v-on="listeners">
				...
				<div :class="itemClass(item)" v-for="(item,index) in options" :key="'action-'+index"
					...
					<div class="mvi-acionsheet-content"
						...
						<m-icon data-placement="left"
							v-if="(iconType(item.icon) || iconUrl(item.icon)) && item.placement!='right'"
							:type="iconType(item.icon)" :url="iconUrl(item.icon)" :spin="iconSpin(item.icon)"
							:size="iconSize(item.icon)" :color="iconColor(item.icon)" />

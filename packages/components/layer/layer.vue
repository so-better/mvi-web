<template>
	<transition :name="(animation?animation:'mvi-layer')" @before-enter="beforeEnter" @enter="enter"
		...
		<div v-if="firstShow" v-show="layerShow" v-on="listeners" :class="['mvi-layer',fixed?'mvi-layer-fixed':'']"
			...
			<div :class="wrapperCls" :style="wrapperStyle">
				<m-triangle v-if="showTriangle" ref="triangle" class="mvi-layer-triangle" :placement="trianglePlacement"
					:background="background" :border-color="(border&&borderColor?borderColor:background)"
					size="0.14rem"></m-triangle>
				<slot></slot>
			</div>
		</div>
	</transition>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			//添加事件
			$dap.event.on(window,`resize.layer_${this._uid}`, this.resizeSet)
			$dap.event.on(window,`click.layer_${this._uid}`, this.hideLayer)
		},
		methods: {
			//窗口变化时处理
			resizeSet() {
			...
			hideLayer(event) {
				if (this.layerShow && this.firstShow && this.closable) {
					if ($dap.element.isContains(this.$el, event.target) || $dap.element.isContains(this.getTargetEl(),
							event.target)) {
						return
					}
		...
		beforeDestroy() {
			$dap.event.off(window,`resize.layer_${this._uid} click.layer_${this._uid}`)
		}
	}
</script>

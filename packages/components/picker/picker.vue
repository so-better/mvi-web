<template>
    <div class="mvi-picker" v-on="listeners">
        ...
        <div v-if="!loading" class="mvi-picker-content" :style="contentStyle" ref="content" @touchmove="contentTouchMove">
            <div v-for="(column,index) in computedOptions" :key="'picker-column-'+index" :class="['mvi-picker-items',column.className || '']" ref="items" :style="columnStyle(column,index)" @touchstart="touchstart($event,index)" @touchmove="touchmove" @touchend="touchend" @mousedown="mousedown($event,index)">
...
export default {
    ...
    mounted() {
        ...
        $dap.event.on(
            document.body,
            `mousemove.picker_${this._uid}`,
            this.mousemove
        )
        $dap.event.on(
            document.body,
            `mouseup.picker_${this._uid}`,
            this.mouseup
        )
    },
    methods: {
    ...
    beforeDestroy() {
        $dap.event.off(
            document.body,
            `mousemove.picker_${this._uid} mouseup.picker_${this._uid}`
        )
    }
}
</script>

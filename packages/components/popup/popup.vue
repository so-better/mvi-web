<template>
	<overlay :show="show" @show="overlayShow" @hide="overlayHide" :use-padding="usePadding" :z-index="zIndex"
		@click.self="hide" :color="overlayColor?overlayColor:null" :timeout="timeout" :local="local">
		<transition :name="'mvi-slide-'+placement" @before-enter="beforeEnter" @enter="enter" @after-enter="afterEnter"
			...
			<div v-if="firstShow" v-show="popupShow" :class="popupClass" :style="popupStyle" v-on="listeners">
				...
				<div v-if="showTimes" :class="['mvi-popup-times','mvi-popup-times-'+timesPlacement]">
					<Icon @click="hidePopup" :type="iconType" :url="iconUrl" :spin="iconSpin" :size="iconSize"
						:color="iconColor" />
				</div>
				<!-- 正文内容 -->

<template>
    <div class="mvi-tabs" v-on="listeners">
        ...
        <div ref="headers" :data-type="type" :class="['mvi-tabs-header',(type=='default'&&border)?'mvi-tabs-header-border':'']" :style="headersStyle">
            ...
            <!-- 选项卡头部卡片 -->
            <div :data-type="type" :class="headerClass(item,index)" v-for="(item,index) in children" :key="'header-'+index" @click="clickHeader(item,index)" :style="headerStyle(item,index)" :disabled="item.disabled">

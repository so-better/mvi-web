<template>
    <m-modal v-on="listeners" v-model="show" :footer-padding="false" @hide="modalHide" @hidding="modalHidding" @hidden="modalHidden" :width="computedWidth" :z-index="computedZIndex" :radius="computedRadius" :local="computedLocal" :use-padding="computedUsePadding" :animation="computedAnimation" @show="modalShow" @showing="modalShowing" @shown="modalShown" :timeout="computedTimeout" :overlay-color="computedOverlayColor">
        ...
        <template v-slot:default v-if="contentShow">
            ...
            <div v-if="type == 'Prompt'" :class="['mvi-dialog-input',computedMessage?'mvi-dialog-input-mt':'']">
                <input ref="input" :type="computedInput.type" :placeholder="computedInput.placeholder" :maxlength="computedInput.maxlength" :class="inputClass" :style="inputStyle" :autofocus="computedInput.autofocus" v-model.trim="computedValue" @focus="inputFocus" @blur="inputBlur" />

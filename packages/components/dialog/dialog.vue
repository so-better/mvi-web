<template>
    <m-modal v-on="listeners" :show="show" :footer-padding="false" @hide="modalHide" @hidding="modalHidding" @hidden="modalHidden" :width="computedWidth" :z-index="computedZIndex" :radius="computedRadius" :local="computedLocal" :use-padding="computedUsePadding" :animation="computedAnimation" @show="modalShow" @showing="modalShowing" @shown="modalShown" :timeout="computedTimeout" :modal-color="computedIos?'rgba(255,255,255,.95)':''" :overlay-color="computedOverlayColor">
        ...
        <template v-slot:default v-if="contentShow">
            ...
            <div v-if="type == 'prompt'" :class="['mvi-dialog-input',(!computedIos && computedMessage)?'mvi-dialog-input-mt':'']">
                <input ref="input" :type="computedInput.type" :placeholder="computedInput.placeholder" :maxlength="computedInput.maxlength" :class="inputClass" :style="inputStyle" :autofocus="computedInput.autofocus" v-model.trim="computedValue" @focus="inputFocus" @blur="inputBlur" :inputmode="computedInputMode" />
    ...
    computed: {
        ...
        computedValue: {
            get() {
                let value = ''
                if (
                    (typeof this.input.value == 'string' && this.input.value) ||
                    typeof this.input.value == 'number'
                ) {

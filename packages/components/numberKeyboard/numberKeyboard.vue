<template>
    <m-overlay ref="overlay" :show="show" @show="overlayShow" @hide="overlayHide" :z-index="zIndex" :color="overlayColor" :timeout="timeout" @click.self="hide" :local="local" :use-padding="usePadding">
            ...
            <div ref="keyboard" class="mvi-number-keyboard" v-if="firstShow" v-show="keyboardShow" :style="boardStyle" v-on="listeners">
                <div class="mvi-number-keyboard-left">
                    <template v-for="(item,index) in numbers">
                        <div :class="leftNumberClass(item)" :key="'number-'+index" v-text="item" @click="numberClick(item)" v-if="showDecimal?true:(item != '.')" :data-decimal="''+showDecimal"></div>
                    </template>
                </div>
